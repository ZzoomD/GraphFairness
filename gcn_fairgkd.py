...
from graphfairness.methods import FairGKD 
...
def args_parser():
    ...
    parser.add_argument('--tem', type=float, default=0.5, help='Temperature for ContLoss.')
    parser.add_argument('--lr_w', type=float, default=0.025, help='AdaWeight learning rate.')
    parser.add_argument('--gamma', type=float, default=0.25, help='AdaWeight gamma.')
...
def run(args):
    ...
    fairgkd = FairGKD(model=model,
                      ...
                      lr_w=args.lr_w,
                      gamma=args.gamma)
    fairgkd.train(fair_dataset, args.epochs, validation=True)

    """
    Evaluation
    """
    results = fairgkd.evaluate(fair_dataset)
