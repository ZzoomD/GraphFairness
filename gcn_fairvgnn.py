...
from graphfairness.methods import FairVGNN
...
def args_parser():
    ...
    parser.add_argument('--ratio', type=float, default=1, help='empower coefficient for mask regularization.')
...
def run(args):
    ...
    Train model
    """
    g_lr, d_lr, e_lr, c_lr = 1e-3, 1e-3, 1e-2, 1e-2
    clip_e, K = 1, 10
    d_epochs, g_epochs, c_epochs = 5, 10, 5
    fairgnn = FairVGNN(model, g_lr=g_lr, d_lr=d_lr, e_lr=e_lr, c_lr=c_lr, clip_e=clip_e, K=K, d_epochs=d_epochs, g_epochs=g_epochs, c_epochs=c_epochs, ratio=args.ratio)
    fairgnn.train(fair_dataset, args.epochs)
