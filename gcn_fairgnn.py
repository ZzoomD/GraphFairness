...
def args_parser():
    ...
    parser.add_argument('--alpha', type=float, default=4, help='empower coefficient for covariance constraint.')
    parser.add_argument('--beta', type=float, default=0.01, help='empower coefficient for adversarial training.')
    parser.add_argument('--sens_number', type=int, default=200, help='The number of sensitive attributes.')
...
def run(args):
    ...
    Train model
    """
    fairgnn = FairGNN(model, nfeat=args.nfeat, nclass=args.nclass, nhid=args.nhid, dropout=args.dropout)
    fairgnn.train(fair_dataset, args.epochs, alpha=args.alpha, beta=args.beta, sens_number=args.sens_number)

    """
    evaluation
    """
    results = fairgnn.evaluate(fair_dataset)
